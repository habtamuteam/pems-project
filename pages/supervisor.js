...
export default function SupervisorDashboard() {
  ...
  async function fetchRequests() {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('/api/requests?stage=Supervisor', {
        headers: {
  ...
  async function handleAction(id, decision) {
    ...
    try {
      ...
      const res = await fetch('/api/requests', {
        ...
        body: JSON.stringify({ id, role: 'Supervisor', decision }),
      });
  ...
  return (
    <div style={{ padding: 20, maxWidth: 800, margin: '0 auto' }}>
      <h1>Supervisor Dashboard - Pending Requests</h1>
      ...
      {actionStatus && <p style={{ fontWeight: 'bold', color: '#007961' }}>{actionStatus}</p>}
